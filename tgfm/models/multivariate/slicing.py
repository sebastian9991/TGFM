"""This is taken directly from https://github.com/galilai-group/lejepa/blob/main/lejepa/multivariate/slicing.py.

`pip install lejepa` points to no registry.
...
from tgfm.models.multivariate.univariate import UnivariateTest
...
class SlicingUnivariateTest(torch.nn.Module):
    ...
    def __init__(
        self,
        univariate_test: UnivariateTest,
        num_slices: int,
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        with torch.no_grad():
            ...
            A = torch.randn(size=proj_shape, generator=g, device=x.device)
            A /= A.norm(p=2, dim=0)
        ...
        if self.reduction == 'mean':
            ...
            raise ValueError(f'Unknown reductions: {self.reduction}')
