"""GraphCLIP in the TGFM pipeline: released-checkpoint adapter and from-scratch MM model.

Two classes share one import path onto the GraphCLIP repo (third_party/GraphCLIP,
commit ce0ace8):

    GraphCLIPAdapter  released checkpoint, TAG zero-shot / robustness evals.
    GraphCLIPMM       random init, pretrained on MM-Graph by mm_main.py with
                      GraphCLIP's graph tower (models/gt.py) and loss
                      (models/dp.py); no released weights are loaded.

The remainder of this docstring describes GraphCLIPAdapter.
...
def graphclip_on_path() -> str:
    """Put the GraphCLIP repo root on sys.path (GRAPHCLIP_ROOT, else third_party/GraphCLIP)."""
    gc_root = os.path.abspath(os.environ.get('GRAPHCLIP_ROOT', 'third_party/GraphCLIP'))
    if not os.path.isdir(os.path.join(gc_root, 'models')):
        raise FileNotFoundError(
            f'GraphCLIP repo not found at {gc_root}; clone it there or set GRAPHCLIP_ROOT.'
        )
...
class GraphCLIPMM(nn.Module):
    """GraphCLIP pretrained from scratch on MM-Graph (G-T or G-I).

    Graph tower: GraphCLIP's GPS (models/gt.py), randomly initialized, built
    with GraphCLIP's Base settings from the config (1024 channels, 12 layers,
    pe_dim 8, multihead attention, RWPE-32 in batch.pe). Its forward returns
    (mlp([mean-pool || center]), mlp2(center)); the first is the graph
    embedding, exactly as GraphCLIP.encode_graph / eval.py use it.

    Partner tower: GraphCLIP locks its text tower (freeze_text) and aligns to
    the mean-pooled MiniLM output with no projection, so the graph head maps
    into the frozen partner space. MM-Graph ships the partner modality as a
    frozen precomputed feature (T5 for G-T, DINOv2 for G-I), so it is used as is
    and GPS's 2*channels -> 384 head, sized for MiniLM, is rebuilt at the
    partner width (768). Temperature: learnable logit_scale initialized at
    log(1/0.07), as in models/graphclip.py.

    Interface parity with LeGTJEPA for mm_main.py / mm_linear_probe.py:
        forward(batch_g, batch_t=None, image_x=None) -> {'z_g', 'z_p', 'logit_scale'}
        encode_graph(batch) -> (B, partner_dim)
        trainable_parameters()
    """

    # GPS modules that pretraining never sends gradient to: mlp2's output is
    # discarded by the loss, attn_pool and the LoRA pair serve prompt tuning.
    _UNUSED_GPS_MODULES = ('mlp2', 'attn_pool', 'lora_A_mlp', 'lora_B_mlp')

    def __init__(self, args: Any) -> None:
        super().__init__()
        graphclip_on_path()
        from models.gt import GPS

        self.args = args
        self.use_text = args.use_text
        self.partner_dim = args.text_in_dim if args.use_text else args.image_in_dim

        self.graph_model = GPS(
            in_dim=args.graph_in_dim,
            channels=args.graph_hidden_dim,
            out_dim=args.graph_hidden_dim,
            pe_dim=args.graph_pe_dim,
            num_layers=args.graph_num_layers,
            attn_type=args.attn_type,
            attn_kwargs={'dropout': args.attn_dropout},
        )
        self.graph_model.mlp = nn.Sequential(
            nn.Linear(2 * args.graph_hidden_dim, self.partner_dim)
        )
        # Frozen so DDP(find_unused_parameters=False) does not wait on
        # gradients that never arrive.
        for name in self._UNUSED_GPS_MODULES:
            for param in getattr(self.graph_model, name).parameters():
                param.requires_grad = False

        self.logit_scale = nn.Parameter(torch.ones([]) * math.log(1 / 0.07))
        logging.info(
            'GraphCLIPMM from scratch: partner=%s (%d-d, frozen, no projection), '
            'graph_in_dim=%d',
            'text' if self.use_text else 'image',
            self.partner_dim,
            args.graph_in_dim,
        )

    def encode_graph(self, batch: Any) -> Tensor:
        graph_embs, _center_embs = self.graph_model(
    ...
    def forward(
        ...
        partner = batch_t if self.use_text else image_x
        if partner is None:
            raise ValueError(
                'GraphCLIPMM: no partner features passed '
                f'({"batch_t" if self.use_text else "image_x"} is None)'
            )
        return {
